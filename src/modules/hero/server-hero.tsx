...
import { ParameterTypeEnum } from "~/generated/prisma/enums";

// Define the hero parameters as const for type safety
export const heroParameters = [
	{ key: "title", type: ParameterTypeEnum.STRING },
	{ key: "description" },
	{ key: "image" },
...
export const serverHero: ServerModule<typeof heroParameters> = {
	...
	loader: ({ element }) => {
		...
		return (
			<div class="w-full h-dvh bg-base-100 flex flex-col md:flex-row justify-center items-center gap-4">
				{image && (
		...
		return (
			...
			>
				...
				<div class="w-1/2 h-auto md:h-full flex flex-col justify-center items-center gap-4">
					...
					<div class="text-sm text-gray-500 flex items-center gap-2">
						<div class="h-4 w-4">{modulesCount}</div> components available!
					</div>
				</div>
			</div>
		);
